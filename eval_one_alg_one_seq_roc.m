...
% ========================================================================
    ...
    for k = 1:n_total_frames
        ...
        if gt_row < 1 || gt_row > n_gt_rows
            warning('[%s] 帧 %d GT 行越界（row=%d），跳过。', alg_name, f, gt_row);
            continue;
        end
        ...
        if ~isfinite(x) || ~isfinite(y)
            warning('[%s] 帧 %d GT 点无效（x/y 非有限值），跳过。', alg_name, f);
            continue;
        end
        ...
        try
            ...
            if ~isequal(size(S), size(I))
                warning('[%s] 帧 %d 输出尺寸不匹配，I=%s, S=%s，跳过。', ...
                    alg_name, f, mat2str(size(I)), mat2str(size(S)));
                continue;
            end
            ...
            if isempty(pos_this) || isempty(neg_this)
                warning('[%s] 帧 %d 正/负样本为空，跳过。', alg_name, f);
                continue;
            end
